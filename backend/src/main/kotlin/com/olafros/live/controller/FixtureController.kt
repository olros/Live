...
class FixtureController(
    ...
    fun getAllFixtures(): List<FixtureDtoList> {
        return fixtureRepository.findAllWithTimeAfter(OffsetDateTime.now())
            .map { fixture -> fixture.toFixtureDtoList() }
    }
    ...
    fun createNewFixture(@Valid @RequestBody fixture: CreateFixtureDto): ResponseEntity<*> {
        if (!securityService.hasSeasonAccess(fixture.seasonId)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body<Any>(ErrorResponse("You're not allowed to create a fixture in this season"))
        }
        ...
        return when {
            ...
            (awayTeam == null) ->
                ResponseEntity.status(HttpStatus.NOT_FOUND).body<Any>(ErrorResponse("Could not find the away-team"))
            (!isValidTeam(homeTeam, season)) ->
                ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body<Any>(ErrorResponse("Could not find ${homeTeam.name} in this season"))
            (!isValidTeam(awayTeam, season)) ->
                ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body<Any>(ErrorResponse("Could not find ${awayTeam.name} in this season"))
            else -> {
    ...
    fun updateSeasonById(
        ...
        if (fixture == null) return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body<Any>(ErrorResponse("Could not find the fixture to update"))
        ...
        return when {
            (homeTeam == null) -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body<Any>(ErrorResponse("Could not find home-team in this season"))
            (awayTeam == null) -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body<Any>(ErrorResponse("Could not find away-team in this season"))
            else -> {
    ...
    fun deleteFixtureById(@PathVariable fixtureId: Long): ResponseEntity<*> {
        ...
        return when {
            (fixture == null) ->
                ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body<Any>(ErrorResponse("Could not find the fixture to delete"))
            else -> {
                fixtureRepository.delete(fixture)
                ResponseEntity.ok<Any>(SuccessResponse("Fixture successfully deleted"))
            }
        }
    }
}
